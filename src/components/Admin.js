...
const Admin = () => {
  ...
  const handleMusicUpload = async () => {
    try {
      ...
      const docRef = await addDoc(musicCollection, {
        ...
        artist: musicArtist,
        url: musicURL,
        releaseDate: new Date(musicReleaseDate), // Converting to Date object
      });

      // Define newMusicItem here, inside the try block
      const newMusicItem = {
        id: docRef.id, // Use the ID from the newly added document
        title: musicTitle,
        artist: musicArtist,
        url: musicURL,
        releaseDate: musicReleaseDate, // Storing the date as a string
      };

      // Update the musicList state with the new item
      setMusicList((currentList) => [...currentList, newMusicItem]);
  ...
  const handleDeleteMusic = async (id) => {
    try {
      ...
      // Update the musicList state to remove the deleted item
      setMusicList((currentList) =>
        currentList.filter((music) => music.id !== id)
      );
    } catch (error) {
 ...
 */}
      ...
      <section className="section">
        ...
        <div className="music-preview">
          <h3>Live Preview</h3>
          <div>
            <h4>{musicTitle || "Music Title"}</h4>
            <p>Release Date: {musicReleaseDate || "YYYY-MM-DD"}</p>
            {musicURL && (
              <div
                className="iframe-container"
                dangerouslySetInnerHTML={{ __html: musicURL }}
              />
            )}
          </div>
        </div>
